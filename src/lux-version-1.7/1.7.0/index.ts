      ...
      const scripts: NodeScript[] = [
        { name: 'start', command: 'ng serve --public-host=http://localhost:4200', overwrite: true },
        {
          ...
          command:
            'run ' +
            options.project +
            ':app-lint --format=stylish && ng run ' +
            options.project +
            ':spec-lint --format=stylish',
          overwrite: true
        }
      ];
