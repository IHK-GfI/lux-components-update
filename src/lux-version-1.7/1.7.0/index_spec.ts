...
describe('lux-version-1.7.0', () => {
  ...
  describe('[Rule] setupProject', () => {
    it('Sollte Fehler werfen, wenn ein empty Tree genutzt wird', async () => {
      try {
        await runner
          .runExternalSchematicAsync(collectionPath, 'lux-version-1.7.0', appOptions, Tree.empty())
          .toPromise();
        fail();
      } catch (ex) {
    ...
    it('Sollte Fehler werfen, wenn keine Option "project" gesetzt ist', async () => {
      try {
        await runner.runExternalSchematicAsync(collectionPath, 'lux-version-1.7.0', {}, appTree).toPromise();
        fail();
      } catch (ex) {
        // Hier sollte eine Exception geworfen werden.
      }
  ...
  describe('[Rule] updateAngularJson', () => {
    it('Sollte die Datei angular.json aktualisieren', async (done) => {
      ...
      callRule(updateAngularJson(), observableOf(appTree), context).subscribe(
        () => {
          // Nachbedingungen prüfen
  ...
  describe('[Rule] fixCompileError', () => {
    it('Sollte MAT_PLACEHOLDER_GLOBAL_OPTIONS entfernen', async (done) => {
      appTree.overwrite(
        ...
        appTree.readContent('/projects/bar/src/app/app.module.ts').replace(
          'providers: []',
          `providers      : [
                {
      ...
      callRule(fixCompileError({ path: '/projects/bar' }), observableOf(appTree), context).subscribe(
        () => {
          // Nachbedingungen prüfen
          expect(appTree.readContent('/projects/bar/src/app/app.module.ts')).not.toContain(
            'MAT_PLACEHOLDER_GLOBAL_OPTIONS'
          );
          expect(appTree.readContent('/projects/bar/src/app/app.module.ts').replace(/(\s{2,})/g, ' ')).toContain(
            'providers : []'
          );
          done();
        },
  ...
  describe('[Rule] addPackageJsonDependencies', () => {
    it('Sollte die Dependencies hinzufügen', async (done) => {
      ...
      callRule(addPackageJsonDependencies(), observableOf(appTree), context).subscribe(
        () => {
          // Nachbedingungen prüfen
  ...
  describe('[Rule] addPackageJsonDevDependencies', () => {
    it('Sollte die DevDependencies hinzufügen', async (done) => {
      ...
      callRule(addPackageJsonDevDependencies(), observableOf(appTree), context).subscribe(
        () => {
          // Nachbedingungen prüfen
  ...
  describe('[Rule] addPackageJsonScripts', () => {
    it('Sollte die Skripte hinzufügen', async (done) => {
      ...
      expect(appTree.readContent('/package.json')).not.toContain(
        'bar:app-lint --format=stylish && ng run bar:spec-lint --format=stylish'
      );

      callRule(addPackageJsonScripts({ project: 'bar' }), observableOf(appTree), context).subscribe(
        () => {
          // Nachbedingungen prüfen
          expect(appTree.readContent('/package.json')).toContain('ng serve --public-host=http://localhost:4200');
          expect(appTree.readContent('/package.json')).toContain(
            'bar:app-lint --format=stylish && ng run bar:spec-lint --format=stylish'
          );
          done();
        },
