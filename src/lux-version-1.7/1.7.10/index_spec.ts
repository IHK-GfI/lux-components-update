...
describe('lux-version-1.7.10', () => {
  ...
  describe('[Rule] setupProject', () => {
    it('Sollte Fehler werfen, wenn ein empty Tree genutzt wird', async () => {
      try {
        await runner
          .runExternalSchematicAsync(collectionPath, 'lux-version-1.7.10', appOptions, Tree.empty())
          .toPromise();
        fail();
      } catch (ex) {
    ...
    it('Sollte Fehler werfen, wenn keine Option "project" gesetzt ist', async () => {
      try {
        await runner.runExternalSchematicAsync(collectionPath, 'lux-version-1.7.10', {}, appTree).toPromise();
        fail();
      } catch (ex) {
        // Hier sollte eine Exception geworfen werden.
      }
  ...
  describe('[Rule] checkVersions', () => {
    it('Sollte einen Fehler werfen, wenn Version < n - 1', () => {
      ...
      callRule(checkVersions(), observableOf(appTree), context).subscribe(
        ...
        (reason) =>
          expect(reason.toString()).toContain('Dieser Generator benötigt allerdings die (neuere) Version 1.7.9.')
      );
    });
    ...
    it('Sollte einen Fehler werfen, wenn Version > n - 1', () => {
      ...
      callRule(checkVersions(), observableOf(appTree), context).subscribe(
        (success) => expect(success).toBeUndefined(),
        (reason) =>
          expect(reason.toString()).toContain('Dieser Generator benötigt allerdings die (ältere) Version 1.7.9.')
      );
    });
  ...
  describe('[Rule] updatePackageJson', () => {
    ...
    it('Sollte die Dependency "lux-components" auf Version 1.7.10 setzen', async (done) => {
      callRule(updatePackageJson(), observableOf(appTree), context).subscribe(
        () => {
          expect(appTree.readContent('/package.json')).toContain('"lux-components": "1.7.10"');
          done();
        },
        (reason) => expect(reason).toBeUndefined()
      );
  ...
  describe('[Rule] updateLUXStylesSCSS', () => {
    it('Sollte die Datei luxstyles.scss ersetzen', async (done) => {
      ...
      callRule(updateLuxStylesScss({ path: '/projects/bar' }), observableOf(appTree), context).subscribe(
        () => {
  ...
  describe('[Rule] updateLuxAppFooterButtonInfoConstructorParams', () => {
    ...
    it('Sollte die Reihenfolge korrekt anpassen', async (done) => {
      ...
      callRule(
        updateLuxAppFooterButtonInfoConstructorParams({ path: '/projects/bar/' }),
        ...
        () => {
          ...
          expect(
            appComponentContent.indexOf(
              `new LuxAppFooterButtonInfo('label1', 'cmd1', 'primary', true, false, 'fa-check')`
            )
          ).toBeGreaterThan(-1);
          expect(
            appComponentContent.indexOf(
              `new LuxAppFooterButtonInfo('label2', 'cmd2', 'primary', false, false, 'fa-check')`
            )
          ).toBeGreaterThan(-1);
          expect(
            appComponentContent.indexOf(
              `new LuxAppFooterButtonInfo('label3', 'cmd3', 'primary', true, false, 'fa-check')`
            )
          ).toBeGreaterThan(-1);
          expect(
            appComponentContent.indexOf(
              `new LuxAppFooterButtonInfo('label4', 'cmd4', 'primary', false, true, 'fa-check', 1, true, false)`
            )
          ).toBeGreaterThan(-1);
          expect(
            appComponentContent.indexOf(
              `new LuxAppFooterButtonInfo('label(5)', 'cmd5', 'primary', true, false, 'fa-check');`
            )
          ).toBeGreaterThan(-1);
          done();
        },
