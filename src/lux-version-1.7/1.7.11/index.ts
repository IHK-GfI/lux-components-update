...
export function renameLuxButtonProperty(options: any) {
  return (tree: Tree, context: SchematicContext) => {
    logInfoWithDescriptor(
      `Benenne die Property ${chalk.redBright('luxRoundButton')} zu ${chalk.redBright('luxRounded')} um.`
    );
    return waitForTreeCallback(tree, () => {
      let fileCount = 0;
...
export function renameLuxCardContentExpanded(options: any) {
  return (tree: Tree, context: SchematicContext) => {
    logInfoWithDescriptor(
      `Benenne das Tag ${chalk.redBright('lux-card-content-expanded')} zu ${chalk.redBright(
...
export function todosForUser(): Rule {
  return (tree: Tree, context: SchematicContext) => {
    runInstallAndLogToDos(
      context,
      `Alle Components unter ${chalk.blueBright(
        'LuxFormModule'
      )} wurden auf eine Baseline gebracht.\r\n${TAB}Bitte prüfen Sie in Ihren Views, ob die Ansichten noch korrekt sind.`,
      `Die Components ${chalk.blueBright('LuxTile')}, ${chalk.blueBright('LuxCard')}, ${chalk.blueBright(
        'LuxPanel'
      )} und ${chalk.blueBright(
        'LuxStepper'
      )} haben in der mobilen Ansicht ca. 50% ihrer Margins und Paddings verloren.\r\n` +
        `${TAB}Bitte prüfen Sie sicherheitshalber Ihre Applikation in den xs- und sm-Queries.`,
      `Die Breite der Component ${chalk.blueBright(
        'LuxTile'
      )} hat in der mobilen Ansicht nun eine Breite von 140px (vorher 150px), bitte den entsprechenden Queries überprüfen.`,
      `Die Properties ${chalk.redBright('luxDashboardLink')}, ${chalk.redBright(
        'luxDashboardLinkTitle'
      )} und ${chalk.redBright('luxOpenLinkBlank')} wurden von ${chalk.blueBright('LuxAppHeader')} entfernt.`,
