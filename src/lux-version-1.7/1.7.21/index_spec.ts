...
describe('lux-version-1.7.20', () => {
    ...
    describe('[Rule] setupProject', () => {
        it('Sollte Fehler werfen, wenn ein empty Tree genutzt wird', () => {
            expect(() => runner.runSchematic('lux-version-1.7.21', {}, Tree.empty()))
    ...
    describe('[Rule] checkVersions', () => {
        ...
        it('Sollte einen Fehler werfen, wenn Version < n - 1', () => {
            ...
            callRule(checkVersions(), observableOf(appTree), context).subscribe(
                ...
                (reason) => expect(reason.toString()).toContain('Dieser Generator benötigt allerdings die (neuere) Version 1.7.20.'));
        });
        ...
        it('Sollte einen Fehler werfen, wenn Version > n - 1', () => {
            ...
            callRule(checkVersions(), observableOf(appTree), context).subscribe(
                (success) => expect(success).toBeUndefined(),
                (reason) => expect(reason.toString()).toContain('Dieser Generator benötigt allerdings die (ältere) Version 1.7.20.'));
        });
