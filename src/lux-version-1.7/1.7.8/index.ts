...
export function updateInterfaceNames(options: any): Rule {
  return (tree: Tree, context: SchematicContext) => {
    logInfoWithDescriptor(
      `` + 'Setze die I-Präfixe für die Klassen "LuxMessage", "LuxMessageCloseEvent" und "LuxMessageChangeEvent".'
    );
...
export function todosForUser(): Rule {
  return (tree: Tree, context: SchematicContext) => {
    runInstallAndLogToDos(
      context,
      `Die Properties ${chalk.redBright('luxNoServerFilter')}, ${chalk.redBright(
        'luxNoServerPagination'
      )} und ${chalk.redBright('luxNoServerSort')} aus dem Interface ${chalk.blueBright(
        'ILuxTableHttpDao'
