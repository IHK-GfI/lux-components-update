...
describe('lux-version-1.7.8', () => {
  ...
  describe('[Rule] setupProject', () => {
    it('Sollte Fehler werfen, wenn ein empty Tree genutzt wird', async () => {
      try {
        await runner
          .runExternalSchematicAsync(collectionPath, 'lux-version-1.7.8', appOptions, Tree.empty())
          .toPromise();
        fail();
      } catch (ex) {
    ...
    it('Sollte Fehler werfen, wenn keine Option "project" gesetzt ist', async () => {
      try {
        await runner.runExternalSchematicAsync(collectionPath, 'lux-version-1.7.8', {}, appTree).toPromise();
        fail();
      } catch (ex) {
        // Hier sollte eine Exception geworfen werden.
      }
  ...
  describe('[Rule] updateLuxAppMenuLeft', () => {
    ...
    it('Das alte Appheadermenü sollte durch das neue Appheadermenü ersetzt sein.', () => {
      ...
      callRule(updateLuxAppMenuLeft({ path: '/projects/bar/src' }), observableOf(appTree), context)
        .toPromise()
        .then(
          () => {
            ...
            expect(content).toContain(
              '<lux-side-nav-item luxLabel="Home" luxIconName="fa-home" (luxClicked)="goToHome()" luxTagId="home-menu-item" ></lux-side-nav-item>'
            );
            expect(content).toContain(
              '<lux-side-nav-item luxLabel="Komponenten" luxIconName="fa-gears" (luxClicked)="goToComponents()" luxTagId="cmp-menu-item" ></lux-side-nav-item>'
            );
            expect(content).toContain(
              '<lux-side-nav-item luxLabel="Master-Detail" luxIconName="fa-th-list" (luxClicked)="goToMasterDetail()" luxTagId="md-menu-item" ></lux-side-nav-item>'
            );
            expect(content).toContain(
              '<lux-side-nav-item luxLabel="Stepper" luxIconName="fa-chevron-circle-right" (luxClicked)="goToStepper()" luxTagId="stepper-menu-item" ></lux-side-nav-item>'
            );
            expect(content).toContain('</lux-side-nav>');
          },
          (reason) => {
  ...
  describe('[Rule] updateLuxList', () => {
    ...
    it('Das Attribut "luxItems" sollte gelöscht sein', () => {
      ...
      // Änderungen durchführen
      callRule(updateLuxList({ path: '/projects/bar/src' }), observableOf(appTree), context)
        .toPromise()
        .then(
          () => {
            // Nachbedingungen prüfen
  ...
  describe('[Rule] updateInterfaceNames', () => {
    ...
    it('Die I-Präfixe für die Klassen LuxMessage, LuxMessageCloseEvent und LuxMessageChangeEvent sollten gesetzt sein.', () => {
      // Änderungen durchführen
      callRule(updateInterfaceNames({ path: '/projects/bar/src' }), observableOf(appTree), context)
        .toPromise()
        .then(
          () => {
            // Nachbedingungen prüfen
  ...
  describe('[Rule] updateHammerConfig', () => {
    ...
    it('LuxHammerConfig sollte in LuxComponentsHammerConfig umbenannt sein.', () => {
      // Änderungen durchführen
      callRule(updateHammerConfig({ path: '/projects/bar/src' }), observableOf(appTree), context)
        .toPromise()
        .then(
          () => {
            // Nachbedingungen prüfen
  ...
  describe('[Rule] updateLuxTab', () => {
    ...
    it('Das Attribut "luxText" sollte in "luxTitle" umbenannt sein', () => {
      ...
      // Änderungen durchführen
      callRule(updateLuxTab({ path: '/projects/bar/src' }), observableOf(appTree), context)
        .toPromise()
        .then(
          () => {
            // Nachbedingungen prüfen
            expect(appTree.readContent('/projects/bar/src/app/app.component.html')).toContain(
              'luxTitle="Hallo Welt 1"'
            );
            expect(appTree.readContent('/projects/bar/src/app/app.component.html')).toContain(
              '[luxTitle]="Hallo Welt 2"'
            );
            expect(appTree.readContent('/projects/bar/src/app/app.component.html')).toContain(
              '[(luxTitle)]="Hallo Welt 3"'
            );
            expect(appTree.readContent('/projects/bar/src/app/app.component.html')).toContain(
              '(luxTitle)="Hallo Welt 4"'
            );
  ...
  describe('[Rule] updateLuxMenu', () => {
    ...
    it('Das Attribut "luxIconName" sollte in "luxMenuIconName" umbenannt sein', () => {
      ...
      // Änderungen durchführen
      callRule(updateLuxMenu({ path: '/projects/bar/src' }), observableOf(appTree), context)
        .toPromise()
        .then(
          () => {
            // Nachbedingungen prüfen
            expect(appTree.readContent('/projects/bar/src/app/app.component.html')).toContain(
              'luxMenuIconName="fa-ellipsis-h"'
            );
            expect(appTree.readContent('/projects/bar/src/app/app.component.html')).toContain(
              '[luxMenuIconName]="fa-user"'
            );
            expect(appTree.readContent('/projects/bar/src/app/app.component.html')).toContain(
              '[(luxMenuIconName)]="fa-check"'
            );
            expect(appTree.readContent('/projects/bar/src/app/app.component.html')).toContain(
  ...
  describe('[Rule] checkVersions', () => {
    ...
    it('Sollte einen Fehler werfen, wenn Version !== n - 1', () => {
      ...
      callRule(checkVersions(), observableOf(appTree), context)
        .toPromise()
        .then(
          (success) => {
  ...
  describe('[Rule] addAttributeTest', () => {
    ...
    it('Das Attribut "luxTest" sollte enthalten sein', () => {
      ...
      // Änderungen durchführen
      callRule(addAttributeTest({ path: '/projects/bar/src' }), observableOf(appTree), context)
        .toPromise()
        .then(
          () => {
            // Nachbedingungen prüfen
  ...
  describe('[Rule] updateAttributeTest', () => {
    ...
    it('Das Attribut "updateAttributeTest" sollte geändert sein.', () => {
      ...
      // Änderungen durchführen
      callRule(updateAttributeTest({ path: '/projects/bar/src' }), observableOf(appTree), context)
        .toPromise()
        .then(
          () => {
            // Nachbedingungen prüfen
