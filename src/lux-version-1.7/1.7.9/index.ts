...
export function updateStylesSCSS(): Rule {
    return (tree: Tree, context: SchematicContext) => {
        ...
        iterateFilesAndModifyContent(tree, '/', (filePath: string, content: string) => {
            deleteLineFromFile(tree, context, filePath, '~font-awesome/css/font-awesome.css');
            writeLinesToFile(tree, context, filePath,
...
export function updatePropertyNames(options: any): Rule {
    return (tree: Tree, context: SchematicContext) => {
        logInfoWithDescriptor('Aktualisiere Templates, um "luxMargin" anstelle von "luxMargins" für LuxIconComponents zu nutzen.');
...
export function todosForUser(): Rule {
    return (tree: Tree, context: SchematicContext) => {
        runInstallAndLogToDos(context,
            `Bitte aktualisieren Sie die folgende Zeile in der Klasse ${chalk.blueBright('ApplicationConfiguration')} vom UI-Service (Proxy):\r\n` +
            `${TAB}Alt: ${chalk.redBright('"/fontawesome-webfont(\\\\\\\\.[a-f0-9]{20})?\\\\\\\\..*"')}\r\n` +
            `${TAB}Neu: ${chalk.redBright('"/(fa-solid|fa-brand|fa-regular|fa-light)-([0-9]{2,4})?(\\.[a-f0-9]{20})?\\.(woff|woff2|ttf)?"')}`,
            ...
            `Bitte prüfen Sie die Margins und Paddings der ${chalk.blueBright('LuxIconComponents')} innerhalb Ihrer Applikation.\r\n` +
            `${TAB}Sie können die Margins und Paddings einer ${chalk.blueBright('LuxIconComponent')} nun über die Properties ${chalk.redBright('luxMargin')} und ${chalk.redBright('luxPaddingMargin')} steuern.`,
