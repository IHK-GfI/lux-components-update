...
describe('lux-version-1.7.9', () => {
  ...
  beforeEach(async () => {
    ...
    const schematic = runner.engine.createSchematic('lux-version-1.7.9', collection);
  ...
  describe('[Rule] setupProject', () => {
    it('Sollte Fehler werfen, wenn ein empty Tree genutzt wird', async () => {
      try {
        await runner
          .runExternalSchematicAsync(collectionPath, 'lux-version-1.7.9', appOptions, Tree.empty())
          .toPromise();
        fail();
      } catch (ex) {
    ...
    it('Sollte Fehler werfen, wenn keine Option "project" gesetzt ist', async () => {
      try {
        await runner.runExternalSchematicAsync(collectionPath, 'lux-version-1.7.9', {}, appTree).toPromise();
        fail();
      } catch (ex) {
        // Hier sollte eine Exception geworfen werden.
      }
  ...
  describe('[Rule] updatePackageJson', () => {
    ...
    it('Sollte fontawesome-free v.5.7.2 als Dependency hinzufügen', () => {
      ...
      callRule(updatePackageJson(), observableOf(appTree), context)
        .toPromise()
        .then(
          () => {
  ...
  describe('[Rule] updateStylesSCSS', () => {
    ...
    it('Sollte den alten font-awesome import entfernen und die neuen imports hinzufügen', () => {
      ...
      // Änderungen durchführen
      callRule(updateStylesSCSS(), observableOf(appTree), context)
        .toPromise()
        .then(
          () => {
            // Nachbedingungen prüfen
  ...
  describe('[Rule] updatePropertyNames', () => {
    ...
    it('Sollte Properties "luxMargins" zu "luxMargin" umbenennen', () => {
      ...
      // Änderungen durchführen
      callRule(updatePropertyNames({ path: '/projects/bar/src' }), observableOf(appTree), context)
        .toPromise()
        .then(
          () => {
            // Nachbedingungen prüfen
