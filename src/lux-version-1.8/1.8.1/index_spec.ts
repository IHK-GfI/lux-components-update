...
describe('lux-version-1.8.1', () => {
  ...
  beforeEach(async () => {
    ...
    const schematic = runner.engine.createSchematic('lux-version-1.8.1', collection);
  ...
  describe('[Rule] setupProject', () => {
    it('Sollte Fehler werfen, wenn ein empty Tree genutzt wird', async () => {
      try {
        await runner
          .runExternalSchematicAsync(collectionPath, 'lux-version-1.8.1', appOptions, Tree.empty())
          .toPromise();
        fail();
      } catch (ex) {
    ...
    it('Sollte Fehler werfen, wenn keine Option "project" gesetzt ist', async () => {
      try {
        await runner.runExternalSchematicAsync(collectionPath, 'lux-version-1.8.1', {}, appTree).toPromise();
        fail();
      } catch (ex) {
        // Hier sollte eine Exception geworfen werden.
      }
  ...
  describe('[Rule] checkVersions', () => {
    it('Sollte einen Fehler werfen, wenn Version < n - 1', () => {
      ...
      callRule(checkVersions(), observableOf(appTree), context).subscribe(
        ...
        (reason) =>
          expect(reason.toString()).toContain('Dieser Generator benötigt allerdings die (neuere) Version 1.8.0.')
      );
    });
    ...
    it('Sollte einen Fehler werfen, wenn Version > n - 1', () => {
      ...
      callRule(checkVersions(), observableOf(appTree), context).subscribe(
        (success) => expect(success).toBeUndefined(),
        (reason) =>
          expect(reason.toString()).toContain('Dieser Generator benötigt allerdings die (ältere) Version 1.8.0.')
      );
    });

    it('Sollte keinen Fehler werfen, wenn Version === n - 1', () => {
      addDependencyToPackageJson(appTree, 'lux-components', '1.8.0');
      Object.defineProperty(process.versions, 'node', {
        get: () => '10.0.0'
      });
  ...
  describe('[Rule] updatePackageJson', () => {
    ...
    it('Sollte die Dependency "lux-components" auf Version 1.8.1 setzen', async (done) => {
      callRule(updatePackageJson(), observableOf(appTree), context).subscribe(
        () => {
          expect(appTree.readContent('/package.json')).toContain('"lux-components": "1.8.1"');
          done();
        },
        (reason) => expect(reason).toBeUndefined()
      );
