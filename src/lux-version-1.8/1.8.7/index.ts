...
export function checkVersions(): Rule {
  return (tree: Tree, context: SchematicContext) => {
    ...
    return waitForTreeCallback(tree, () => {
      const minimumLuxComponentsVersion = '1.8.6';
...
export function todosForUser(): Rule {
  return (tree: Tree, context: SchematicContext) => {
    ...
    runInstallAndLogToDos(
      context,
      `Manuelle Schritte aus dem Update Guide (https://github.com/IHK-GfI/lux-components/wiki/update-guide-1.8#version-${version}) durchführen!`
    );
    return tree;
  };
}
