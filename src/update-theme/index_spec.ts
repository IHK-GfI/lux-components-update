...
import { findThemeDir, normalizeVersion, updateTheme } from './index';
...
describe('update-theme', () => {
  ...
  beforeEach(async () => {
    ...
    const schematic = runner.engine.createSchematic('update-theme', collection);
  ...
  describe('[Rule] copyThemeFiles', () => {
    it('Sollte die richtigen Themedateien kopieren', async (done) => {
      ...
      callRule(updateTheme(testOptions), observableOf(appTree), context).subscribe(
        () => {
